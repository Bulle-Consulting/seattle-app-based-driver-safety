...
const TEAL = "#0d9488";
const SEV: Record<string, string> = { fatal: "#f59e0b", injury: "#f97316", robbery: "#8b5cf6", assault: "#3b82f6", policy: "#64748b" };
const PLT: Record<string, string> = { Uber: "#94a3b8", Lyft: "#e879f9", DoorDash: "#f87171", "Amazon Flex": "#f59e0b" };
...
const Tip = ({ active, payload, label }: any) => {
  ...
  return (
    <div className="chart-tooltip">
      <p className="font-medium text-[#e2e8f0] mb-0.5">{label}</p>
...
function Modal({ incident: inc, onClose }: { incident: Incident; onClose: () => void }) {
  const c = SEV[inc.severity] ?? "#64748b";
  useEffect(() => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-6" style={{ background: "rgba(0,0,0,.6)", backdropFilter: "blur(3px)" }} onClick={onClose}>
      <div className="bg-[#1e293b] rounded-lg border border-[#334155] p-5 max-w-md w-full" onClick={e => e.stopPropagation()}>
        <div className="flex items-start justify-between mb-3">
          <div>
            <div className="flex items-center gap-1.5 mb-1"><SeverityBadge severity={inc.severity} /><StatusBadge status={inc.status} />
              {(inc.category === "policy_regulatory" || inc.category === "legal_sentencing") && <span className="text-[8px] bg-[#334155] text-[#94a3b8] px-1.5 py-0.5 rounded">{inc.category === "policy_regulatory" ? "Policy" : "Legal"}</span>}
            </div>
            <div className="text-[13px] font-semibold text-[#e2e8f0]">{inc.type}</div>
            <div className="text-[12px] font-medium" style={{ color: TEAL }}>{inc.neighborhood}</div>
          </div>
          <button onClick={onClose} className="text-[#334155] hover:text-[#64748b] p-1 rounded transition-colors"><X size={14} /></button>
        </div>
        <div className="space-y-2.5 text-[11px]">
          <div className="bg-[#0f172a] rounded p-3"><div className="section-label mb-1">Description</div><p className="text-[#e2e8f0] leading-relaxed">{inc.description}</p></div>
          <div className="grid grid-cols-2 gap-2">
            <div className="bg-[#0f172a] rounded p-2.5"><div className="section-label mb-0.5">Date</div><div className="tabular-nums text-[#e2e8f0]">{new Date(inc.date).toLocaleDateString("en-US", { month: "long", day: "numeric", year: "numeric" })}</div></div>
            <div className="bg-[#0f172a] rounded p-2.5"><div className="section-label mb-0.5">Platform</div><div className="text-[#e2e8f0]">{inc.platform}</div></div>
          </div>
          {inc.victim && <div className="bg-[#0f172a] rounded p-2.5"><div className="section-label mb-0.5">Victim</div><div className="text-[#e2e8f0]">{inc.victim}</div></div>}
          <div className="bg-[#0f172a] rounded p-2.5"><div className="section-label mb-0.5">Location</div><div className="text-[#94a3b8] text-[10px]">{inc.address}</div><div className="tabular-nums text-[9px] text-[#64748b] mt-0.5">{inc.lat.toFixed(5)}, {inc.lng.toFixed(5)}</div></div>
          <div className="bg-[#0f172a] rounded p-2.5"><div className="section-label mb-0.5">Source</div>
            {inc.sourceUrl ? (
            ...
            ) : <div className="text-[#94a3b8]">{inc.source}</div>}
          </div>
        </div>
        <div className="mt-4 flex gap-2">
          ...
          <a href={`https://www.google.com/maps/search/?api=1&query=${inc.lat},${inc.lng}`} target="_blank" rel="noopener noreferrer"
            className="flex items-center gap-1 text-[10px] text-[#94a3b8] hover:text-[#e2e8f0] bg-[#0f172a] border border-[#334155] px-3 py-2 rounded transition-colors">
...
export default function Dashboard() {
  ...
  const pltData = stats ? Object.entries(stats.byPlatform || {}).map(([n, v]) => ({ name: n, value: Number(v), fill: PLT[n] ?? "#334155" })).sort((a, b) => b.value - a.value) : [];
  ...
  return (
    <div className="dashboard-layout">
      ...
      <div className="main-content">
        <Header title="App-Based Driver Safety Steward" subtitle="App-Based Driver Safety Steward · Bulle Cloud · Safety Steward" />
        <main className="flex-1 p-5 space-y-5">

          {(sevFilter || yearFilter !== "All") && (
            <div className="flex items-center gap-2 text-[10px] text-[#94a3b8]">
              <Filter size={10} className="text-[#64748b]" />
              {sevFilter && <button onClick={() => setSevFilter(null)} className="flex items-center gap-1 border border-[#334155] bg-[#1e293b] px-2 py-0.5 rounded hover:bg-[#334155]">{sevFilter} <X size={8} /></button>}
              {yearFilter !== "All" && <button onClick={() => setYearFilter("All")} className="flex items-center gap-1 border border-[#334155] bg-[#1e293b] px-2 py-0.5 rounded hover:bg-[#334155]">{yearFilter} <X size={8} /></button>}
              <button onClick={() => { setSevFilter(null); setYearFilter("All"); }} className="text-[#64748b] hover:text-[#94a3b8] ml-1">Clear</button>
            </div>
          )}
          ...
          <div className="flex items-center gap-1.5 flex-wrap">
            ...
            {years.map(y => (
              <button key={y} onClick={() => setYearFilter(y)}
                className={`text-[10px] px-2 py-0.5 rounded border transition-colors ${yearFilter === y ? "bg-[#0d9488] text-white border-transparent font-medium" : "bg-[#1e293b] border-[#334155] text-[#94a3b8] hover:border-[#475569]"}`}>
                {y}
              </button>
            ))}
            <div className="ml-auto flex items-center gap-2">
              ...
              <button onClick={() => setShowHeatmap(h => !h)}
                ...
                style={{ background: showHeatmap ? TEAL : "#334155", borderColor: showHeatmap ? TEAL : "#475569" }}>
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-6 gap-3">
            {[
              { label: "Crime Total", value: stats?.crimeTotal ?? 0, color: "#e2e8f0", sev: null },
              ...
              <div key={label} data-testid={`kpi-${label.toLowerCase().replace(/\s+/g,'-')}`}
                ...
                className={`kpi-card bg-[#1e293b] border border-[#334155] rounded-md p-4 ${sev ? "cursor-pointer" : ""} ${sevFilter === sev ? "active-filter" : ""}`}>
                <div className="text-[10px] font-medium text-[#94a3b8] mb-1">{label}</div>
          ...
          <div className="grid grid-cols-1 xl:grid-cols-3 gap-4">
            <div className="xl:col-span-2 bg-[#1e293b] border border-[#334155] rounded-md overflow-hidden">
              <div className="flex items-center justify-between px-4 py-2 border-b border-[#334155]">
                <span className="text-[11px] font-medium text-[#e2e8f0]">Incident Map</span>
            ...
            <div className="bg-[#1e293b] border border-[#334155] rounded-md flex flex-col">
              <div className="px-4 py-2 border-b border-[#334155] flex items-center justify-between">
                <span className="text-[11px] font-medium text-[#e2e8f0]">Recent</span>
                ...
                {incL ? Array(5).fill(0).map((_, i) => <div key={i} className="px-4 py-3"><Skeleton className="h-3 w-full" /></div>) :
                  recent.map(inc => (
                    <div key={inc.id} data-testid={`incident-card-${inc.id}`}
                      ...
                      className={`incident-row px-4 py-3 cursor-pointer hover:bg-[#0f172a] ${inc.id === selectedId ? "selected" : ""}`}>
                      <div className="flex items-center justify-between mb-0.5">
                        <div className="flex items-center gap-1">
                          <SeverityBadge severity={inc.severity} />
                          {(inc.category === "policy_regulatory" || inc.category === "legal_sentencing") && <span className="text-[7px] bg-[#334155] text-[#64748b] px-1 rounded">{inc.category === "policy_regulatory" ? "Policy" : "Legal"}</span>}
                        </div>
                        <span className="text-[9px] text-[#64748b] tabular-nums">{new Date(inc.date).toLocaleDateString("en-US", { month: "short", day: "numeric" })}</span>
                      </div>
                      <div className="flex items-center justify-between">
                        <div><div className="text-[11px] font-medium text-[#e2e8f0]">{inc.type}</div><div className="text-[9px] text-[#94a3b8]">{inc.neighborhood} · {inc.platform}</div></div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-4">
            <div className="md:col-span-2 bg-[#1e293b] border border-[#334155] rounded-md p-4">
              <div className="flex items-center justify-between mb-3">
                <span className="text-[11px] font-medium text-[#e2e8f0]">Monthly Trend (crime only)</span>
                <span className="text-[9px] text-[#64748b]">Jan 2024 – Present</span>
              </div>
              <ResponsiveContainer width="100%" height={155}>
                <LineChart data={moData}>
                  ...
                  <Tooltip content={<Tip />} />
                  <Line type="monotone" dataKey="incidents" stroke={TEAL} strokeWidth={1.5} dot={{ fill: TEAL, r: 2.5, strokeWidth: 0 }} activeDot={{ r: 4, fill: SEV.fatal, stroke: "#1e293b", strokeWidth: 1.5 }} name="Incidents" />
                </LineChart>
              </ResponsiveContainer>
            </div>

            <div className="bg-[#1e293b] border border-[#334155] rounded-md p-4">
              <span className="text-[11px] font-medium text-[#e2e8f0]">By Severity</span>
              ...
              <div className="mt-1 space-y-px">
                {sevData.map(d => (
                  <button key={d.name} onClick={() => toggleSev(d.name.toLowerCase())} className="w-full flex items-center justify-between text-[9px] py-0.5 px-1 rounded hover:bg-[#0f172a]">
                    <span className="flex items-center gap-1"><span className="w-1.5 h-1.5 rounded-full" style={{ background: d.color }} /><span className="text-[#94a3b8]">{d.name}</span></span>
                    <span className="tabular-nums font-medium text-[#e2e8f0]">{d.value}</span>
                  </button>
                ))}
              </div>
            </div>
            ...
            <div className="bg-[#1e293b] border border-[#334155] rounded-md p-4">
              <span className="text-[11px] font-medium text-[#e2e8f0]">By Platform</span>
              <ResponsiveContainer width="100%" height={100} className="mt-2">
                <BarChart data={pltData} layout="vertical">
                  <XAxis type="number" tick={{ fontSize: 8, fill: "#64748b" }} axisLine={false} tickLine={false} />
                  <YAxis dataKey="name" type="category" tick={{ fontSize: 9, fill: "#64748b" }} axisLine={false} tickLine={false} width={60} />
                  <Tooltip content={<Tip />} />
          ...
          <div className="bg-[#1e293b] border border-[#334155] rounded-md p-4">
            <div className="flex items-center justify-between mb-3">
              <span className="text-[11px] font-medium text-[#e2e8f0]">Hotspot Neighborhoods</span>
              <span className="text-[9px] text-[#64748b]">Crime incidents only</span>
            </div>
                ...
                return (
                  <div key={n.name}>
                    <div className="flex items-center justify-between text-[10px] mb-1">
                      <span className={i === 0 ? "font-medium text-[#e2e8f0]" : "text-[#94a3b8]"}>{n.name}</span>
                      <span className="tabular-nums font-medium" style={{ color: i === 0 ? SEV.fatal : "#64748b" }}>{n.value}</span>
                    </div>
          ...
          <div className="bg-[#1e293b] border border-[#334155] rounded-md overflow-hidden">
            <div className="px-4 py-2.5 border-b border-[#334155] flex items-center justify-between">
              <span className="text-[11px] font-medium text-[#e2e8f0]">All Incidents <span className="text-[#64748b] tabular-nums font-normal ml-1">{filtered.length}</span></span>
              <span className="text-[9px] text-[#64748b]">Click row for details · linked sources open in new tab</span>
            </div>
                  ...
                  {incL ? Array(4).fill(0).map((_, i) => <tr key={i}>{Array(8).fill(0).map((_, j) => <td key={j} className="px-4 py-3"><Skeleton className="h-3 w-14" /></td>)}</tr>)
                    : filtered.slice().sort((a, b) => b.date.localeCompare(a.date)).map(inc => (
                      <tr key={inc.id} data-testid={`row-incident-${inc.id}`}
                        onClick={() => { setSelectedId(inc.id!); setModal(inc); }}
                        className={`incident-row cursor-pointer hover:bg-[#0f172a] transition-colors ${inc.id === selectedId ? "selected" : ""}`}>
                        ...
                        <td className="px-4 py-2.5 font-medium text-[#e2e8f0] max-w-[120px] truncate">{inc.type}</td>
                        <td className="px-4 py-2.5 text-[9px] text-[#94a3b8]">{inc.category === "policy_regulatory" ? "Policy" : inc.category === "legal_sentencing" ? "Legal" : "Crime"}</td>
                        <td className="px-4 py-2.5"><SeverityBadge severity={inc.severity} /></td>
                        <td className="px-4 py-2.5 text-[#94a3b8] max-w-[120px] truncate">{inc.neighborhood}</td>
                        ...
                        <td className="px-4 py-2.5 max-w-[130px] truncate">
                          ...
                          ) : <span className="text-[#64748b] text-[9px]">{inc.source}</span>}
                        </td>
                      </tr>
                    ))
                  }
          ...
          <div className="text-[9px] text-[#64748b] pb-2 space-y-1">
            <div className="flex items-center justify-between">
