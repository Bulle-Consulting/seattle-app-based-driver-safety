...
const TEAL = "#26A69A";
const SEV: Record<string, string> = { fatal: "#E8A317", injury: "#E0772B", robbery: "#8B6DC7", assault: "#4A8DE0", policy: "#6D7A8F" };
...
function Modal({ incident: inc, onClose }: { incident: Incident; onClose: () => void }) {
  const c = SEV[inc.severity] ?? "#6D7A8F";
  useEffect(() => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-6" style={{ background: "rgba(11,17,32,0.6)", backdropFilter: "blur(3px)" }} onClick={onClose}>
      <div className="bg-[#151d2e] rounded-lg border border-[#1F2937] p-5 max-w-md w-full" onClick={e => e.stopPropagation()}>
        <div className="flex items-start justify-between mb-3">
          <div>
            <div className="flex items-center gap-1.5 mb-1"><SeverityBadge severity={inc.severity} /><StatusBadge status={inc.status} />
              {(inc.category === "policy_regulatory" || inc.category === "legal_sentencing") && <span className="text-[8px] bg-[#1F2937] text-[#A3AEC0] px-1.5 py-0.5 rounded">{inc.category === "policy_regulatory" ? "Policy" : "Legal"}</span>}
            </div>
        ...
        <div className="space-y-2.5 text-[11px]">
          ...
          <div className="grid grid-cols-2 gap-2">
            <div className="bg-[#101827] rounded p-2.5"><div className="section-label mb-0.5">Date</div><div className="tabular-nums text-[#F5F5F5]">{new Date(inc.date).toLocaleDateString("en-US", { month: "long", day: "numeric", year: "numeric" })}</div></div>
          ...
          <div className="bg-[#101827] rounded p-2.5"><div className="section-label mb-0.5">Source</div>
            ...
            ) : <div className="text-[#A3AEC0]">{inc.source}</div>}
          </div>
        </div>
        <div className="mt-4 flex gap-2">
          ...
          <a href={`https://www.google.com/maps/search/?api=1&query=${inc.lat},${inc.lng}`} target="_blank" rel="noopener noreferrer"
            className="flex items-center gap-1 text-[10px] text-[#A3AEC0] hover:text-[#F5F5F5] bg-[#101827] border border-[#1F2937] px-3 py-2 rounded transition-colors">
...
export default function Dashboard() {
  ...
  const pltData = stats ? Object.entries(stats.byPlatform || {}).map(([n, v]) => ({ name: n, value: Number(v), fill: PLT[n] ?? "#1F2937" })).sort((a, b) => b.value - a.value) : [];
      ...
      <div className="main-content">
        ...
        <main className="flex-1 p-5 space-y-5">
          ...
          <div className="flex items-center gap-1.5 flex-wrap">
            ...
            {years.map(y => (
              <button key={y} onClick={() => setYearFilter(y)}
                className={`text-[10px] px-2 py-0.5 rounded border transition-colors ${yearFilter === y ? "bg-[#26A69A] text-white border-transparent font-medium" : "bg-[#151d2e] border-[#1F2937] text-[#A3AEC0] hover:border-[#475569]"}`}>
                {y}
              </button>
            ))}
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-6 gap-3">
            {[
              { label: "Crime Total", value: stats?.crimeTotal ?? 0, color: "#F5F5F5", sev: null },
          ...
          <div className="grid grid-cols-1 xl:grid-cols-3 gap-4">
            ...
            <div className="bg-[#151d2e] border border-[#1F2937] rounded-md flex flex-col">
              <div className="px-4 py-2 border-b border-[#1F2937] flex items-center justify-between">
                <span className="text-[11px] font-medium text-[#F5F5F5]">Recent</span>
                ...
                {incL ? Array(5).fill(0).map((_, i) => <div key={i} className="px-4 py-3"><Skeleton className="h-3 w-full" /></div>) :
                  recent.map(inc => (
                    <div key={inc.id} data-testid={`incident-card-${inc.id}`}
                      ...
                      className={`incident-row px-4 py-3 cursor-pointer hover:bg-[#0b1120] ${inc.id === selectedId ? "selected" : ""}`}>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-4">
            ...
            <div className="bg-[#151d2e] border border-[#1F2937] rounded-md p-4">
              ...
              <div className="mt-1 space-y-px">
                {sevData.map(d => (
                  <button key={d.name} onClick={() => toggleSev(d.name.toLowerCase())} className="w-full flex items-center justify-between text-[9px] py-0.5 px-1 rounded hover:bg-[#0b1120]">
                    <span className="flex items-center gap-1"><span className="w-1.5 h-1.5 rounded-full" style={{ background: d.color }} /><span className="text-[#A3AEC0]">{d.name}</span></span>
                    <span className="tabular-nums font-medium text-[#F5F5F5]">{d.value}</span>
                  </button>
                ))}
              </div>
            </div>
                  ...
                  {incL ? Array(4).fill(0).map((_, i) => <tr key={i}>{Array(8).fill(0).map((_, j) => <td key={j} className="px-4 py-3"><Skeleton className="h-3 w-14" /></td>)}</tr>)
                    : filtered.slice().sort((a, b) => b.date.localeCompare(a.date)).map(inc => (
                      <tr key={inc.id} data-testid={`row-incident-${inc.id}`}
                        onClick={() => { setSelectedId(inc.id!); setModal(inc); }}
                        className={`incident-row cursor-pointer hover:bg-[#0b1120] transition-colors ${inc.id === selectedId ? "selected" : ""}`}>
                        ...
                        <td className="px-4 py-2.5 font-medium text-[#F5F5F5] max-w-[120px] truncate">{inc.type}</td>
                        <td className="px-4 py-2.5 text-[9px] text-[#A3AEC0]">{inc.category === "policy_regulatory" ? "Policy" : inc.category === "legal_sentencing" ? "Legal" : "Crime"}</td>
                        <td className="px-4 py-2.5"><SeverityBadge severity={inc.severity} /></td>
                        <td className="px-4 py-2.5 text-[#A3AEC0] max-w-[120px] truncate">{inc.neighborhood}</td>
